...
class DaysMonthTokenParserTest {

    @InjectMocks
    private DaysMonthTokenParser underTest;
    ...
    public void should_parse_valid_star_command() {
        ...
        assertEquals(1, result.getRangeStart());
        assertEquals(31, result.getRangeEnd());
    ...
    @Test
    public void should_parse_and_fail_out_of_range_lower() {
        Exception exception = assertThrows(ParserException.class,
                () -> underTest.parse("0"));
        assertEquals(
                "Number '0' should be in range '1' and '31'.",
                exception.getMessage());
    }
    ...
    public void should_parse_and_fail_out_of_range_higher() {
        Exception exception = assertThrows(ParserException.class,
                () -> underTest.parse("32"));
        assertEquals(
                "Number '32' should be in range '1' and '31'.",
                exception.getMessage());
    }

    @Test
    public void should_parse_and_fail_with_star_divide_by_range_expr() {
        Exception exception = assertThrows(ParserException.class,
                () -> underTest.parse("*/1-2"));
        assertEquals(
                "Expression '*/{number}-{number}' is not allowed.",
